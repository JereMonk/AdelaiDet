...
def extend_detectron2_config(_C: CN) -> CN:
    _C.INPUT.CROP_CAR = False
    _C.INPUT.CROP_CAR_EXTEND_BBOX = 0.0
    _C.INPUT.SATURATION = [1.0, 1.0]
    _C.INPUT.BRIGHTNESS = [1.0, 1.0]
    _C.INPUT.CONTRAST = [1.0, 1.0]

    _C.DATASETS.TASK = ""
    _C.DATASETS.SUBCATS = []
    _C.DATASETS.MIN_SIZE_SCRATCH = 0.0
    _C.DATASETS.MIN_SIZE_DENT = 0.0
    # _C.DATASETS.TRAIN  # Train datasets
    # _C.DATASETS.TEST   # Validation datasets
    _C.DATASETS.TRAIN_VERSION = (0, 1)
    _C.DATASETS.TEST_VERSION = (0, 0)

    # Mode must be in [crop, uncrop]."
    # 'crop' crops the ImageDataset and compares it to the PredictionInstance
    # 'incrop' uncrops the PredictionInstance and compares it to the ImageDataset
    _C.TEST.PROCESS_MODE = "uncrop"

    _C.TEST.EVALUATOR_TYPES = []
    _C.TEST.DOWNSIZE_LONGER_SIDE = 1500
    _C.TEST.CONF_THRESHOLDS = [0.25]
    _C.TEST.GROUPPING_THRESH = 0.01
    _C.TEST.DISTRIBUTED = True

    _C.TEST.INSTANCE2SEM = CN()
    _C.TEST.INSTANCE2SEM.CONTOUR_IGNORE = 0

    _C.TEST.CUSTOMF1 = CN()
    _C.TEST.CUSTOMF1.IOU_THRESHOLDS = (('scratch', 0.01), ('dent', 0.1))
    _C.TEST.CUSTOMF1.WEIGHTS = (('scratch', 0.5), ('dent', 0.5))
    _C.TEST.CUSTOMF1.AVG_METRICS = True

    # TO REMOVE, kept only because
    _C.INPUT.CONTOUR_IGNORE_TEST = 1  # temp, to remove
    _C.INPUT.HORIZONTAL_FLIP_PROB = 0.5  # temp, to remove
    _C.INPUT.LIGHTNING_STDEV = 1.0  # temp, to remove
    _C.DATASETS.VERSION = (100, 100)  # temp, to remove after rush

    return _C

def get_config_blendmask():
    ...
    return extend_detectron2_config(_C.clone())
